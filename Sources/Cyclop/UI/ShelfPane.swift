...
struct ShelfPane: View {
    ...
    private var dropHint: some View {
        RoundedRectangle(cornerRadius: 14, style: .continuous)
            .strokeBorder(
                isTargeted ? Theme.text.opacity(0.6) : Theme.hairline,
                style: StrokeStyle(lineWidth: 1.5, dash: [6, 5])
            )
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(isTargeted ? Theme.surface : Theme.clear)
            )
            .overlay(
                Image(systemName: "tray.and.arrow.down.fill")
                    .font(.system(size: 20, weight: .light))
                    .foregroundStyle(isTargeted ? Theme.text : Theme.tertiary)
            )
            .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(isSelected ? Theme.text.opacity(0.18) : (isHovered ? Theme.surfaceHover : Theme.surface))
        )
        .overlay(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .strokeBorder(Theme.text.opacity(isSelected ? 0.55 : 0), lineWidth: 1.5)
